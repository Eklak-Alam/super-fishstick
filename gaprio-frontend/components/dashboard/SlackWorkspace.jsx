...
export default function SlackWorkspace({ isConnected, data }) {
    const [modal, setModal] = useState(false);
    const [formData, setFormData] = useState({});
    const [loading, setLoading] = useState(false);

    const handleAuth = () => window.location.href = `${process.env.NEXT_PUBLIC_API_URL}/auth/slack`;
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-6xl mx-auto">
            <div className="flex justify-between items-center mb-8">
                <div>
                    <h2 className="text-2xl font-bold text-white">Slack Headquarters</h2>
                    <p className="text-sm text-green-400 flex items-center gap-2 mt-1"><span className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse" /> Connected</p>
                </div>
                    ...
                    {data.channels?.length ? data.channels.map(channel => (
                        <div key={channel.id} className="p-4 bg-white/5 rounded-xl border border-white/5 hover:border-orange-500/30 transition-all cursor-pointer group">
                            <div className="flex justify-between items-center mb-2">
                                <p className="text-white font-bold group-hover:text-orange-400 transition-colors">#{channel.name}</p>
                                <span className="flex items-center gap-1 text-[10px] text-zinc-500 bg-black/40 px-2 py-1 rounded-full"><Users size={10}/> {channel.members_count}</span>
                            </div>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                        <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="w-full max-w-lg bg-[#0a0a0a] border border-white/10 rounded-2xl p-6 relative">
                            ...
                            <form onSubmit={sendMessage} className="space-y-4">
                                <div>
                                    <label className="text-xs text-zinc-500 mb-1 block">Select Channel</label>
                                    <select className="input-std" onChange={e => setFormData({...formData, channelId: e.target.value})} required>
                                ...
                                <div>
                                    ...
                                    <textarea placeholder="Type your message..." rows={4} className="input-std" onChange={e => setFormData({...formData, message: e.target.value})} required />
                                </div>
                                <button disabled={loading} className="w-full bg-blue-600 text-white font-bold py-3 rounded-lg hover:bg-blue-500 transition-colors">
